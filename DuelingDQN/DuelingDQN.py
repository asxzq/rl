...
class MLP(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim=128):
        ...
        self.V_s = nn.Linear(hidden_dim, 1)  # 输出层
        self.A_s_a = nn.Linear(hidden_dim, output_dim)
...
class DuelingDQN:
    ...
    def update(self):
        ...
        # 根据下一时刻状态，计算Q策略下一时刻选取的a
        next_a = self.policy_net(next_state_batch).detach().max(1)[1].unsqueeze(1)
        # 根据下一时刻状态，计算Q'策略下一时刻的输出，并且根据Q策略选出的a，进行输出
        next_q_target_values = self.target_net(next_state_batch).detach().gather(1,next_a).squeeze(1)
        # 计算期望的Q值，对于终止状态，此时done_batch[0]=1, 对应的expected_q_value等于reward
        # 形状为 1 * batch_size 后续需要unsqueeze
