...
class CriticNet(nn.Module):
    def __init__(self, s_dim, a_dim):
        ...
        self.fcs = nn.Linear(s_dim, 40)
        self.fcs.weight.data.normal_(0, 0.1)
        self.fca = nn.Linear(a_dim, 40)
    ...
    def forward(self, s, a):
        x = self.fcs(s)
        y = self.fca(a)
        fc = self.fc(F.relu(x + y))
        actions_value = self.out(F.relu(fc))
