...
class Arguments:
    def __init__(self, agent=None, env=None):
        ...
        self.break_step = 2 ** 26  # break training after 'total_step > break_step'
        ...
        self.learning_rate = 1e-5  # 2 ** -14 ~= 6e-5
        self.soft_update_tau = 5e-3  # 2 ** -8 ~= 5e-3

        self.net_dim = 2 ** 9  # the network width
        self.batch_size = self.net_dim * 2  # num of transitions sampled from replay buffer.
        self.repeat_times = 2 ** 3  # collect target_step, then update network
        self.target_step = 1200  # repeatedly update network to keep critic's loss small
        self.max_memo = self.target_step  # capacity of replay buffer
        self.if_per_or_gae = False  # GAE for on-policy sparse reward: Generalized Advantage Estimation.

        '''Arguments for evaluate'''
        self.eval_gap = 2 ** 6  # evaluate the agent per eval_gap seconds
        self.eval_times = 6  # number of times that get episode return in first
...
def train_and_evaluate(args, agent_id=0):
    ...
    while if_train:
        ...
        torch.save(agent.act.state_dict(), "temp_actor.pth")
    torch.save(agent.act.state_dict(), "final_actor.pth")
